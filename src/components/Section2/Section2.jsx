...
// Ícone Oficial do Google Maps (Pin de Localização)
...
export default function Section2() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Distância total que a track precisa percorrer no eixo X.
      // Já inclui o espaçador (.trackEnd) do fim da trilha, então o
      // usuário precisa continuar rolando um pouco depois do último
      // card antes da seção soltar o pin — sem corte abrupto.
      ...
      // 1) Movimento horizontal da track, 100% preso ao scroll.
      master.to(track, { x: () => -getScrollDistance(), ease: 'none' }, 0);

      // 2) Deslocamento diagonal (Y) por card, alternado.
      ...
      // 3) Animações Desacopladas (Gatilhos Independentes)
      cards.forEach((card) => {
        ...
        // A. IMAGEM (Começa assim que o card encosta na tela)
        if (image) {
          gsap.to(image, {
            scale: 1,
            ease: 'none',
            scrollTrigger: {
              ...
              start: 'left 95%',  // Entrou 5% na tela, a imagem já começa a ajustar a escala
              end: 'left 30%',
              scrub: 0.5,
            },
          });
        ...
        // B. LINHAS SVG (Dispara de forma rápida assim que o HEADER aparece)
        if (header && lines.length) {
          gsap.to(lines, {
            ...
            scrollTrigger: {
              trigger: header,
              containerAnimation: master,
              start: 'left 90%',  // Assim que o header entra à direita
              end: 'left 60%',   // Animação curta e ágil: termina rápido enquanto o título se posiciona
              scrub: 0.3,        // Sensação de resposta mais direta
            },
          });
        }

        // C. PALAVRAS DA REVELAÇÃO (Inicia quando o Bloco de Conteúdo entra)
        if (content && words.length) {
          gsap.to(words, {
            ...
            scrollTrigger: {
              trigger: content,
              containerAnimation: master,
              start: 'left 85%',  // Começa logo após a linha dar as caras
              end: 'left 40%',
              scrub: 0.5,
            },
          });
      ...
      // Recalcula o posicionamento exato
